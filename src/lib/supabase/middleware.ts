...
export async function updateSession(request: NextRequest) {
  ...
  if (user && isProtectedRoute && !request.nextUrl.pathname.startsWith("/admin") && !request.nextUrl.pathname.startsWith("/acesso-suspenso")) {
    const { data: usuarioDb } = await supabase.from("usuarios").select("ativo").eq("id", user.id).maybeSingle();
    if (usuarioDb && usuarioDb.ativo === false) {
      await supabase.auth.signOut();
      const url = request.nextUrl.clone();
      url.pathname = "/login";
      url.searchParams.set("error", "Seu acesso foi suspenso. Entre em contato com o administrador.");
      return NextResponse.redirect(url);
    }
  }
